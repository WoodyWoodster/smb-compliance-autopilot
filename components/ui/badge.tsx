...
const badgeVariants = cva(
  "inline-flex items-center justify-center rounded-full border px-2.5 py-0.5 text-xs font-medium w-fit whitespace-nowrap shrink-0 [&>svg]:size-3 gap-1.5 [&>svg]:pointer-events-none focus-visible:border-ring focus-visible:ring-ring/50 focus-visible:ring-[3px] aria-invalid:ring-destructive/20 dark:aria-invalid:ring-destructive/40 aria-invalid:border-destructive transition-[color,box-shadow] overflow-hidden",
  {
    variants: {
      variant: {
        // Teal primary - for positive/completed states
        default:
          "border-transparent bg-primary text-primary-foreground [a&]:hover:bg-primary/90",
        // Coral - for attention/action needed
        coral:
          "border-transparent bg-coral text-coral-foreground [a&]:hover:bg-coral/90",
        secondary:
        ...
        // Success - for completed/positive status
        success:
          "border-transparent bg-success text-success-foreground [a&]:hover:bg-success/90",
        // Warning - for due soon/attention
        warning:
          "border-transparent bg-warning text-warning-foreground [a&]:hover:bg-warning/90",
        // Muted teal outline for subtle positive state
        "teal-outline":
          "border-primary/30 bg-primary/10 text-primary [a&]:hover:bg-primary/20",
        // Coral outline for subtle attention state
        "coral-outline":
          "border-coral/30 bg-coral/10 text-coral [a&]:hover:bg-coral/20",
      },
    },
    defaultVariants: {
      variant: "default",
